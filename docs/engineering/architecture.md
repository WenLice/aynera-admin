...
| API contract (repo) | `elaris-api/docs/api-reference.md` |
| Backend onboarding (repo) | `elaris-api/docs/developer-guide.md` |
...
| **Persistence** | Main entities (`AppUser`, …), `ElarisDbContext`, EF migrations |
...
EF Core **code-first** migrations own application schema. Local Postgres holds an empty `elaris` database until migrations run.
